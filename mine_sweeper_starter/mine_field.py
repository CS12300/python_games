...
class MineField():
    ...
    def reveal(self, x, y):
        ...
        if self.cells[x][y].near_bombs == 0:
            ...
            # to do this
            self.clear_empty_neighbor(x, y)

    def clear_empty_neighbor(self, x, y):
        """
        Recursively clearing out all empty/numbered neighbors of an empty cleared cell
        """
         # Loop through all neighboring cells
        if self.cells[x][y].near_bombs == 0:
            for xneighbor in self.neighbors:
                for yneighbor in self.neighbors:
                    # Skip current cell and any out-of-bounds cells
                    if not ((xneighbor == 0 and yneighbor == 0)
                        ...
                        if ((self.cells[x+xneighbor][y+yneighbor].covered) and
                            (not self.cells[x+xneighbor][y+yneighbor].bomb)):
                            self.cells[x+xneighbor][y+yneighbor].covered = False
                            self.clear_empty_neighbor(x + xneighbor, y + yneighbor)
